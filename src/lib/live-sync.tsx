...
export function LiveSyncProvider({ children }: { children: ReactNode }) {
  ...
  const sendSnapshot = useCallback(
    async (automatic = false) => {
      ...
      try {
        const snapshot = await parseResponse<Snapshot>(await fetch("/api/backup", { cache: "no-store" }));
  ...
  const attachChannel = useCallback(
    (dc: RTCDataChannel) => {
      ...
      dc.onopen = () => {
        ...
        if (room) void post(room, "touch").catch(() => undefined);
        if (heartbeatRef.current) clearInterval(heartbeatRef.current);
        heartbeatRef.current = setInterval(() => {
          const activeRoom = roomRef.current;
          if (activeRoom) void post(activeRoom, "touch").catch(() => undefined);
        }, 20_000);
