...
export async function enrichWithAi(e: LexiconEvidence): Promise<{ fields: AutofillFields; provider: string; ai: boolean }> {
  const key = e.word.toLowerCase();
  const hit = aiCache.get(key);
  if (hit && Date.now() - hit.at < 12 * 3600_000) return { ...hit.value, ai: true };
  const messages = buildPrompt(e);
  // Stay well inside serverless limits (Vercel Hobby = 60 s per request).
