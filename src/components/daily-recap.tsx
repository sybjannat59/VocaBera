...
import { getLocalRecap } from "@/lib/idb";
...
export function DailyRecap() {
  ...
  const load = useCallback(async () => {
    try {
      const local = await getLocalRecap(localDay());
      if (local && (local.day || local.items?.length)) return local;
    } catch {
      /* fallback to api if local DB error */
    }
    try {
      const res = await fetch(`/api/recap?today=${localDay()}`, { cache: "no-store" });
      if (res.ok) return (await res.json()) as RecapData;
    } catch {
      /* ignore */
    }
    return { day: null, activity: null, items: [] };
  }, []);
